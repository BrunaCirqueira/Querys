...
SELECT DISTINCT
		...
		[Valor Total] =
			CASE 
				WHEN N.RcurCode IS NOT NULL THEN A.[Saldo]
			ELSE
...
SELECT DISTINCT * FROM (
	SELECT 
       ...
       [Tipo] = (SELECT G.TransType FROM OJDT G WHERE G.TransId = A.TransId),
       [IR] = 
			CASE 
					WHEN (SELECT G.TransType FROM OJDT G WHERE G.TransId = A.TransId) = '18' 
					THEN (SELECT COUNT(*) FROM OPCH T0 INNER JOIN PCH5 T1 ON T0.DocEntry = T1.AbsEntry WHERE T1.Category = 'P' AND T0.DocNum = (SELECT G.Baseref FROM OJDT G WHERE G.TransId = A.TransId))
			END,         
       [LC] = 
			CASE A.TransType
       ...
       [N.Doc]= 
			CASE A.TransType
					WHEN 18 THEN (SELECT V.DocEntry FROM OPCH V WHERE V.TransId = A.TransId) 
					ELSE '' 
			END,
       [AD] =  
			CASE A.TransType
					WHEN 204 THEN (SELECT G.DocEntry FROM ODPO G WHERE G.TransId = A.TransId) ELSE NULL
			END,
       [LP] = '',
       [Trans. Rec]=null,
       [Duplicata] = 
...
WHERE A.ContraAct <> A.ShortName
AND (A.BalDueDeb <> 0 OR A.BalDueCred <> 0)
AND B.CardType = 'S'
)AS TR
WHERE TR.[IR] = 0 OR TR.IR IS NULL
AND TR.[Tipo]  IN ('18','204','30')
...
SELECT DISTINCT
		...
		B.BPLName,
		B.BPLId
	FROM OADM E, dbo.FxLanctoPeriodico() A
...
WHERE T0.SeqCode <> '1' AND T0.DocStatus = 'O'
...
WHERE 
T2.[Status] = 'O'
AND(T1.WTAmnt IS NOT NULL AND T1.Category = 'p') 

UNION ALL

SELECT *,null,null,(SELECT BPLName FROM ODRF WHERE DocEntry = TransRec), (SELECT BPLId FROM ODRF WHERE DocEntry = TransRec) FROM [dbo].[FlxContasPagar]({?DataIni}, {?DataFinal})
