SELECT 
		...
		T5.[CardName],
		[Duplicata] = CASE WHEN 
								cast(T6.Installmnt as nvarchar(20)) IS NOT NULL THEN 'NF ENTRADA'+' '+ 
								cast(T6.serial as nvarchar(20)) + ' - ' + cast(T3.InstId as nvarchar(20)) + '/' + cast(T6.Installmnt  as nvarchar(20))
						   ELSE 'LC - ' + cast(T3.DocTransId as nvarchar(20))	  
					  END,	  

		[Total da Parcela] = CASE WHEN 
									   CAST(T6.Installmnt as nvarchar(20)) IS NOT NULL THEN T8.LineTotal
								  ELSE T3.SumApplied
							 END,
							 
		[Valor Pago]= CASE WHEN 
								(SELECT COUNT(1) FROM VPM2 WHERE DocNum = T3.DocNum) = 1 THEN T4.DocTotal
						   WHEN CAST(T6.Installmnt as nvarchar(20)) IS NOT NULL THEN T3.SumApplied
						   ELSE T3.SumApplied
					  END,
					  
		[Saldo] = CASE WHEN 
						   (SELECT COUNT(1) FROM VPM2 WHERE DocNum = T3.DocNum) = 1 THEN (T3.SumApplied - T4.DocTotal)
		...
		[DuplicataLink] = CASE WHEN 
								   cast(T6.Installmnt as nvarchar(20)) IS NOT NULL THEN cast(T6.DocEntry as nvarchar(20)) 
							   ELSE cast(T2.TransId as nvarchar(20))	  
						  END,
						  
		[DuplicataTeste] = CASE WHEN 
									cast(T6.Installmnt as nvarchar(20)) IS NOT NULL THEN 'NF'
								ELSE 'LC' 
						   END
